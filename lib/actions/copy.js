...
exports.copy = function (from, to, options, context) {
  to = path.resolve(to);
  options = options || {};
  ...
  files.forEach(file => {
    this._copySingle(file, generateDestination(file), options, context);
  });
};

exports._copySingle = function (from, to, options, context) {
  options = options || {};
  ...
  if (context) {
    to = ejs.render(to, context, options);
  }
